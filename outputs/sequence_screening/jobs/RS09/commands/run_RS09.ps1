...
$ExportedCaseManifestPath = [System.IO.Path]::GetFullPath((Join-Path $RepoRoot "nnUNet_raw_sequence_screening\Dataset409_LiverTumor_PORTAL\case_manifest.csv"))
...
python -m scripts.dataset.export_sequence_screening_dataset --manifest $ManifestPath --study-config $StudyConfigPath --experiment-id RS09 --nnunet-raw $NNUNetRawPath --roi-column roi_mask_dilated_path --roi-mode masked --crop-margin-mm 20.0
if ($LASTEXITCODE -ne 0) { throw "Step failed: RS09 export source dataset (exit code $LASTEXITCODE)" }
python -m scripts.dataset.generate_splits_json --manifest $ManifestPath --study-config $StudyConfigPath --experiment-id RS09 --nnunet-preprocessed $NNUNetPreprocessedPath --exported-case-manifest $ExportedCaseManifestPath
...
python -m scripts.dataset.export_sequence_screening_targets --manifest $ManifestPath --study-config $StudyConfigPath --experiment-id RS09 --out-dir $TargetsDir --roi-column roi_mask_dilated_path --roi-mode masked --crop-margin-mm 20.0
