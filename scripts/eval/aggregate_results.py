...
from scripts.common.common import ensure_dir


METRICS = ["dice", "hd95", "lesion_recall", "lesion_precision", "lesion_f1", "volume_relative_error"]


def target_in_source_set(row: pd.Series) -> bool:
    target = str(row.get("target_seq", "")).strip()
    source_target_set = str(row.get("source_target_set", "")).strip()
    if not target or not source_target_set:
        return False
    members = {item.strip() for item in source_target_set.split("+") if item.strip()}
...
def main():
    ...
    if "source_target_set" in internal.columns or "source_primary_target" in internal.columns:
        same_mask = pd.Series(False, index=internal.index)
        if "source_target_set" in internal.columns:
            same_mask = same_mask | internal.apply(target_in_source_set, axis=1)
        if "source_primary_target" in internal.columns:
            has_primary = internal["source_primary_target"].fillna("").astype(str).str.len() > 0
            same_mask = same_mask | (
                has_primary & (internal["source_primary_target"].astype(str) == internal["target_seq"].astype(str))
            )
        fallback_mask = same_mask.copy()
        if "source_primary_target" in internal.columns:
            fallback_mask = fallback_mask | (
                internal["source_primary_target"].fillna("").astype(str).str.len() == 0
            )
        if "source_target_set" in internal.columns:
            fallback_mask = fallback_mask & (internal["source_target_set"].fillna("").astype(str).str.len() == 0)
        same_mask = same_mask | (
            fallback_mask & (internal["source_seq"].astype(str) == internal["target_seq"].astype(str))
        )
        same_domain = internal[same_mask].copy()
        cross_domain = internal[~same_mask].copy()
    else:
