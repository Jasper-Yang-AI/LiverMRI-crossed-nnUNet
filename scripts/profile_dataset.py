...
def main():
    ...
    for _, row in experiment_catalog.iterrows():
        lines.append(
            f"- {row['experiment_id']} (dataset {int(row['dataset_id'])}): {row['description']} | train {int(row['n_train_patients'])} patients / {int(row['n_train_cases'])} cases | source {row['source_members']}"
        )
